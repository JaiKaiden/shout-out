<?php
    ...
    if ($_GET['action'] == 'postShout') {
        if (!$_POST['shoutContent']) {
            echo "Your shout is empty!";
          } else if (strlen($_POST['shoutContent']) > 140) {
              echo "Your shout is too long!";
          } else {
            mysqli_query($link, "INSERT INTO shouts (`shout`, `userid`, `datetime`) VALUES ('". mysqli_real_escape_string($link, $_POST['shoutContent'])."', ". mysqli_real_escape_string($link, $_SESSION['id']).", NOW())");
            echo "1";
          }
    }
?>